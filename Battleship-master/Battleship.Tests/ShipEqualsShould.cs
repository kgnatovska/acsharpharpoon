using NUnit.Framework;
using Battleship;

namespace Battleship.Tests
{
    public class ShipEqualsShould
    {
        [Test]
        public void ReturnFalse_ComparingDifferentXAndY()
        {
            var ship1 = new PatrolBoat(x: 1, y: 2);
            var ship2 = new PatrolBoat(x: 2, y: 2);
            var ship3 = new PatrolBoat(x: 1, y: 1);
            
            Assert.AreNotEqual(ship1, ship2);
            Assert.AreNotEqual(ship1, ship3);
        }

        [Test]
        public void ReturnTrue_ComparingWithSameXAndY()
        {
            var ship1 = new PatrolBoat(x: 1, y: 2);
            var ship2 = new PatrolBoat(x: 1, y: 2);

            Assert.AreEqual(ship1, ship2);
        }

        [Test]
        public void ReturnFalse_ComparingWithDifferentLength()
        {
            var ship1 = new PatrolBoat(x: 1, y: 2);
            var ship2 = new Cruiser(x: 1, y: 2);

            Assert.AreNotEqual(ship1, ship2);
        }

        [Test]
        public void ReturnTrue_ComparingWithSameDirection()
        {
            var ship1 = new PatrolBoat(x: 1, y: 2, direction: Directions.Vertiacal);
            var ship2 = new PatrolBoat(x: 1, y: 2, direction: Directions.Vertiacal);

            Assert.AreEqual(ship1, ship2);
        }

        [Test]
        public void ReturnTrue_ComparingWithDifferentDirectionsAndLength1()
        {
            var ship1 = new PatrolBoat(x: 1, y: 2, direction: Directions.Horizontal);
            var ship2 = new PatrolBoat(x: 1, y: 2, direction: Directions.Vertiacal);

            Assert.AreEqual(ship1, ship2);
        }

        [Test]
        public void ReturnFalse_ComparingWithDifferentDirectionsAndLengthAbove1()
        {
            var ship1 = new Cruiser(x: 1, y: 2, direction: Directions.Horizontal);
            var ship2 = new Cruiser(x: 1, y: 2, direction: Directions.Vertiacal);

            Assert.AreNotEqual(ship1, ship2);
        }

        [Test]
        public void ReturnTrue_ComparingCruiserWithSameXAndY()
        {
            var ship1 = new Cruiser(x: 1, y: 2);
            var ship2 = new Cruiser(x: 1, y: 2);

            Assert.AreEqual(ship1, ship2);
        }
    }
}